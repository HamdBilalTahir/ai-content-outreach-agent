...
> ### Outbound port — Drift sync D2: the referral fork machinery
>
> - **What changed:** Rewrote `referralTransfer.ts` against four upstream commits (2026-08-05) that turned a light re-route into a real fork, added `resolveActiveOutboundChat` to `services/chat.ts`, and **closed a Phase 9 deferral the plan wrongly recorded as delivered**.
> - **Re-split from the announced plan.** D2 was going to be `review_call_transcript`. Reading it showed the new orchestrator calls `handleReferralTransfer` with `force_same_line` and `archive_reason`, which this port's signature did not have — so the fork machinery is the prerequisite, and it lands first. The review pipeline becomes D3, and everything after shifts by one. Splitting this way keeps each commit coherent and independently testable; stubbing the two parameters to get the review landed first would have violated the port's nothing-is-stubbed rule.
> - **A real gap found in my own port.** `PORT-PLAN.md` claimed Phase 9 closed "`referralTransfer`'s CRM lookup (Phase 7a)". It did not — the file was last touched at Phase 7b¹ and `contactId` was still hardcoded `null`. That mattered more than a missing CRM mirror: the same-line fork keys the new chat's doc id on the HubSpot contact id when it has one, so a stale `null` would have silently changed chat ids. The lookup-or-create is now wired, and the plan is corrected rather than left overstating.
> - **The source chat is now HARD-STOPPED, not labelled.** Previously the source kept its stage and opt-outs and took a `referral_transferred` label. Now both opt-outs are set, pending tasks are cancelled, and the chat is archived with an `archive_reason`. The reasoning is sound — outreach has been re-pointed at the real person, so the wrong-name chat must never be dialed again — but it inverts two assertions the port's tests pinned deliberately, so those tests were rewritten to the new behaviour rather than patched around.
> - **Same-line forks are the point of the change.** "There's no Claira, it's Chris" on the dealership's single number leaves nothing distinct to key a chat on. The new path creates the chat under a custom doc id on the source's line, mirroring the enroll tail inline, and carries the SOURCE's `userId` so an inbound call resolves to the live chat rather than the archived one — which is what `resolveActiveOutboundChat` is for. It is deliberately conservative: it only deviates from the deterministic id when that chat is archived or absent, so an ordinary number behaves exactly as before.
> - **The deterministic key gate is what makes the fork safe.** Name AND company AND a reachable channel, or no chat is created. Same-line is never inferred from a missing email/phone — a name-only referral to someone at a different store ("Andre, over at the Ford store") has no reachable contact, so it must fail rather than fork onto the wrong line. Both halves of that are pinned by tests.
> - **A library-semantics divergence, the same class as the one found at 10e².** `seedReferrerContext` sorts the referrer's call cards chronologically via `str(timestamp)`, which works in Python because a Firestore datetime stringifies to an ISO-ish form. In JS `String(Timestamp)` is `[object Object]` for every card — all keys compare equal and the sort silently degrades to stream order. Sorting on the coerced epoch preserves what the source achieves rather than what it literally writes.
> - **Three dead imports upstream, not ported**: `REFERRAL_TRANSFERRED_LABEL`, `_OUTREACH_NOTES`, and the `chat as svc` alias are all imported by the new source file and used by none of it. The label constant itself stays in `services/chat.ts` — other code still reads it.
> - **One signature divergence, deliberate:** `handleReferralTransfer` takes an options object rather than the source's four positional parameters. Two call sites and the tests were updated; the source's ordering carried no meaning worth preserving at four optionals.
> - **`ChatMemory.campaign_id` widened to `string | null`** — the fork writes an explicit null there, and every reader already coalesces.
> - **Files:** `outbound/services/{referralTransfer,chat,emailReview}.ts`, `outbound/types.ts`, `outbound/tools/reviewCallTranscript.ts`, `outbound/__tests__/services/voiceFoundation.test.ts`, `outbound/__tests__/tools/reviewCallTranscript.test.ts`
> - **Verification:** `tsc --noEmit` clean, `eslint outbound/` clean, **2,308 tests across 56 suites** green (up from 2,291).
>
> ---
>
...
> - **E2E Test needs NINE endpoints, not the four the plan said — plan revision 4.** The survey found the client references twelve API paths; three were already served (`agents/list` and `outbound/agents` from U3, `hubspot/delete-records` through the catch-all). With 709 lines of routes plus a 3,868-line page and client, U6 split: routes first because they can be probed live on their own, and the client is a single file that cannot be split anyway. **This is the fourth phase where my endpoint estimate was low**, always for the same reason — estimating from the plan's original survey instead of re-surveying the phase's own files.
> - **`initiate` is a new route, not the repoint the plan called for, and finding out why was the useful part.** The ported `initiateOutboundWebhookView` has **no auth guard** — correct for what it is, a webhook that is `AllowAny` in Django and called by external lead sources. But it means pointing a browser button at `webhook/initiate-outbound/` would have placed an unauthenticated **lead-enrolment** endpoint one fetch from the UI. I verified rather than assumed: an unauthenticated POST to that path reaches the view and returns its own `"leads array is required"`. The new route adds a session guard for the browser path; the webhook path stays open, which is what an external lead source needs — worth knowing, since anything that reaches it can enrol leads and start calls.
> - **`voice-workers/transcript` is the one route whose source could not be ported.** It calls the **inbound** Django product with a `backendToken`, and this repo neither has that service nor is a port of it — there was nothing to translate. Rewritten against `fetchConversationFromElevenlabs` (backend 7b²d), which reads the same conversation the inbound API was relaying, from where it originates. The caveat is recorded on the route rather than buried: the payload is now ElevenLabs' verbatim, so a field the inbound service _synthesized_ rather than relayed would show as a blank in the UI, not an error.
> - **`trigger-ai` calls `runOutboundTurn` in-process** rather than POSTing to Django's `call-llm-outbound/` with a token. Chat resolution — phone and agent id off the chat doc — is unchanged, and `adminTriggerSource: 'human'` matches what the HTTP endpoint defaulted to. That argument matters: only a human trigger is authoritative on timing, which is exactly right for an operator typing into the E2E screen.
> - **The audio route's Range handling is carried across untouched**, because it is the whole point of the file. Buffering to send an explicit `Content-Length` (without it the browser reports `duration: Infinity` and the progress bar never moves), `Accept-Ranges` to advertise seekability, and honouring Range with 206 so a seek does not snap back to the start. Those three comments in the source read like someone who debugged each symptom in turn. Only the auth and the one ElevenLabs fetch changed.
> - **Six routes needed nothing but the now-standard three substitutions** — `auth()` → `getAuthenticatedUserId()`, `adminDb` → this repo's `db` with its unreachable null guard dropped, `@/lib/chat-utils` → `@/lib/utils`. Query shapes, field writes and arithmetic untouched, including `opt-out`'s careful habit of writing **both** `memory.*` and the top-level flags, which its own comment explains: writing only memory left a stale top-level `true` that overrode the toggle.
> - **Files:** nine `route.ts` under `src/app/api/{outbound,voice-workers,elevenlabs,admin}/…`
> - **Verification:** `tsc --noEmit` clean, `eslint` clean over the ported files (the repo's 5 pre-existing errors in its own admin routes untouched), `next build` exits 0 emitting all nine, backend's 2,291 tests still green, and **all nine probed live: 401 unauthenticated**, with the catch-all confirmed intact for `dnc`, `campaigns` and the webhook path.
>
> ---
>
...
> - **Campaigns is ONE phase, not the three I planned — plan revision 2.** Mapping the import graph before starting showed the split does not exist: `CampaignsClient` (the list) imports `NewCampaignSheet`, which imports all five audience tabs, which import `AudiencePreview`, `SearchableSelect`, and the helpers. The list alone pulls in essentially the whole tree, so a split would have shipped a broken intermediate — the same reasoning that kept the chat-detail suite whole in U2.
> - **The source repo is being edited while this port runs, and that is the finding worth carrying forward.** `ui-admin-panel`'s HEAD moved to `b6fe7ee` (2026-08-05, _"move entire Skills data layer to Admin SDK"_) mid-phase, and four files in scope carry uncommitted ` M` status while `app/api/outbound/{agents,attribution,funnel}/` are untracked entirely. This is backend revision 9 recurring, one degree worse — not just a moving HEAD but a **dirty tree**, so the snapshot being ported cannot be reproduced from git. I noticed because `shared.tsx` lost a firebase import between two greps minutes apart. Porting continues against the working tree (it is the live source of truth and the changes are real improvements), but every phase now re-surveys its own files first.
> - **The re-survey found the Funnel and Attribution Timeline no longer call the Django analytics endpoints** — plan revision 3. They now fetch `/api/outbound/funnel/chat-counts`, `/api/outbound/funnel/drill` and `/api/outbound/attribution/*`, new untracked routes reading Firestore directly through the Admin SDK. Stated plainly: **backend phases 10d¹ and 10d³ ported views the UI has since stopped using.** They remain correct and mounted; U4/U5 will port what the UI actually calls rather than repointing it back.
> - **Two more missing endpoints, both reached through `useOutboundAgents`.** `/api/agents/list` drops the source's company scoping — that panel is multi-tenant and resolves a company id from Django with a `next-auth` token, none of which exists here — and accepts-but-ignores the parameter so ported client code needs no edit. `/api/outbound/agents` narrows the picker to agents owning an outbound skill; its own source comment explains why it is server-side, because the browser client SDK's `collectionGroup` query fails in some environments and silently fell back to listing _every_ agent.
> - **A static route under a catch-all, verified rather than assumed.** `/api/outbound/agents` sits inside the path the backend's catch-all owns. Next resolves static segments ahead of catch-alls, so it wins for that one path — and a live probe confirmed `dnc` and `campaigns` still return 200 through the ported route table. Worth knowing before adding more: any new static path here silently takes precedence.
> - **`@types/papaparse` is declared locally rather than installed**, because this repo has a **pre-existing** peer conflict — `dotenv@^17` against `@browserbasehq/stagehand`'s `^16` via `@langchain/community` — that makes every `npm install` fail without `--legacy-peer-deps`. This also explains an earlier silent no-op install. Forcing a repo-wide resolution change to obtain types for one CSV parser is the wrong trade, so `src/types/papaparse.d.ts` declares only the single `parse` overload actually called.
> - **The campaign detail page answers 200 unauthenticated, and that is correct.** Its `loading.tsx` forces a streamed response, so the status line is committed before the guard resolves; the body is an inert skeleton and the redirect travels in the RSC payload. The list page, which has no `loading.tsx`, 307s normally. Checked the body directly rather than trusting the status code — skeleton plus a `/login` redirect, no data.
> - **Three `react-hooks/exhaustive-deps` directives became plain comments.** They document deliberate dependency omissions, but this repo's eslint has no react-hooks plugin so the directives errored as unknown rules. The intent is preserved in prose rather than deleted.
> - **Files:**
>   - `src/app/admin/outbound/campaigns/**` (page, `[id]`, 4 components, 9 audience modules, `shared.tsx`)
>   - `src/app/api/agents/list/route.ts`, `src/app/api/outbound/agents/route.ts`
>   - `src/types/papaparse.d.ts`, `src/app/admin/SidebarNav.tsx`
> - **Verification:** `tsc --noEmit` clean, `eslint` clean over the ported files, `next build` exits 0 emitting both campaign routes, backend's 2,291 tests still green, and the two new APIs probed live (401 unauthenticated, catch-all unaffected).
>
> ---
>
...
> - **A real divergence was caught and closed, and it is the most interesting thing here.** A `mailto:a@b.com` in a contact's website field has no `://`, so both the source and a direct translation prefix it to `http://mailto:a@b.com`. Python's `urlparse` reads that as host `mailto:a@b.com` — garbage that fails to fetch. **WHATWG `URL` reads the same string as userinfo `mailto:a` plus host `b.com`.** So the port would have fetched a real, unrelated domain, and a phone number listed on _that_ site would have marked the lead website-verified. A wrong answer rather than a missing one, and invisible afterwards: the property is a boolean and nothing records which URL produced it. `normalizeUrl` now refuses any URL that parses with a username or password. Found because a test fixture of mine was wrong — I had asserted the source rejected mailto, and checking what it actually does surfaced the real problem.
> - **A new shape of defect worth naming.** This is not a Python-vs-JS truthiness difference like the `{}`-falsiness and absent-vs-null cases the port has been tracking since Phase 2 — it is a **library-semantics** difference, where two standard URL parsers disagree about a malformed input. `int()` vs `parseInt` in Phase 10a was the same family and benign. This one was not, and it is the kind that a careful reading of the _source_ would never reveal, because the source is correct.
> - **Playwright is not ported; the scraping-provider path is, in full.** The source picks a fetch engine three ways — a configured scraping API, else headless Chromium when installed, else plain requests. Playwright is a ~300MB browser download plus a process lifecycle: the source needs a page counter, a recycle every 25 pages, and a teardown, because Chromium wedges after 25–50 pages on its author's machine. That is a large operational dependency in service of one backfill. **The consequence is stated rather than hidden:** a JS-rendered or Cloudflare-protected site yields `false`, exactly as the source's own `SCRAPER_ENGINE=requests` mode does. `fetchPage` is an injectable seam, so a Playwright fetcher can be added later without touching anything else — and `SCRAPER_PROVIDER` gets JS rendering back today, including the `raw` template escape hatch that works with a provider the source never heard of.
> - **`tel:` links and visible text are UNIONED, not short-circuited.** A `tel:` link is the strongest signal — it is the number the business chose to make clickable — but plenty of sites print a number without linking it and plenty link one they never print. Ported with regexes rather than a DOM parser, which is the source's own BeautifulSoup-unavailable fallback plus the `tel:` extraction; stripping tags leaves **script contents** in the text exactly as `get_text` does, which is how a JSON-LD `telephone` value is picked up. Asserted.
> - **Last-ten matching is what makes the comparison work at all**, since a site writes `(303) 555-1212` and HubSpot stores `+13035551212`. An 11-digit number is only trimmed when it starts with `1`.
> - **`false` is written for a contact with no website, and that is the point.** Absent and `false` are different facts — absent means never checked, `false` means checked and not listed — and the `business_only` gate needs to tell them apart. Which matters more than it sounds: CNAM returns "unknown" for very nearly every number, so this property is what makes `business_only` usable rather than a filter that rejects almost everything.
> - **It flushes every TEN contacts, not every hundred.** Each contact costs a network fetch of someone else's website, so a page of a hundred takes minutes; a small flush means a run killed mid-page keeps most of its progress. Same reason it returns a resumable cursor, and it pages by seek on `hs_object_id` for the same reason as the area-code backfill — HubSpot caps `after` at 10,000 results.
> - **`agentId` is required**, where the source defaults to a hardcoded production agent id — same decision as 10e¹. A literal id in a port is a value that silently rots, and a script that writes to a customer's CRM should not have a default target.
> - **Files:**
>   - `outbound/commands/websiteVerifiedBusiness.ts`, `outbound/commands/index.ts` (registered, now nine)
>   - `outbound/__tests__/commands/websiteVerifiedBusiness.test.ts`, `outbound/__tests__/commands/index.test.ts`
> - **Verification:** 2,291 tests across 56 suites (52 new), `tsc --noEmit` clean, `eslint` clean.
>
> ---
>
> ### Outbound agent port — Phase 10e¹: the backfills and the operational runners
>
> - **What changed:** Ported eight of the nine `management/commands/` as `outbound/commands/{optoutBackfills,hubspotBackfills,index}.ts`. Re-surveyed the directory first, per revision 9's lesson — nine commands, 851 lines, unchanged since 10d³. `backfill_website_verified_business` is deferred to 10e², because its Playwright/scraping-provider fetch engine needs its own decision.
> - **The Django command wrapper is not ported; the arguments are.** `add_arguments` defaults, clamps, and `--dry-run` semantics are the substance and live in the function signatures. `BaseCommand`, argv parsing, and the `manage.py` entry are dropped — Django supplies a runner, this repo has none, and taking a `tsx` dependency to invent one is outside a port of the application. `commands` in `index.ts` is the registry that replaces `manage.py <name>`, keyed on the source's own command names so the set stays greppable against `ls management/commands/`. Asserted as a list.
> - **Every backfill is idempotent and SET-ONLY**, which is what lets them run against production without a maintenance window. `backfillOptoutFlags` seeds a missing key and raises a false to true, and **never clears a top-level opt-out**: the chat doc is the trustworthy record the gates read, so if it says the customer opted out and memory disagrees, the chat doc wins. Reversing that would let a stale memory field silently re-open a channel the customer closed. It also reads `block_phone` alongside `phone_opt_out`, because both spellings are live on real data.
> - **A `dryRun` reports the counters a real run would produce.** The read path is identical and only the write is skipped, so the numbers can be trusted. `backfillEmailOptoutChatFlags` goes further and _resolves the chat_ during a dry run, so its count reflects real pending writes rather than every suppression entry — a dry run that miscounted would be worse than none, because it would be believed.
> - **Each HubSpot backfill writes exactly ONE property.** A batch update replaces the properties it is given, so including a second field read slightly stale would overwrite a rep's edit made in the meantime. Asserted on the request body.
> - **`backfillAaaiAreaCode` pages by SEEK, not by offset.** HubSpot caps `after` at 10,000 results, so an offset walk **truncates a larger audience while reporting success** — the silent kind of wrong. Sorting by `hs_object_id` ascending and filtering `hs_object_id > lastId` has no cap, and the last id comes back so a run killed partway through resumes where it stopped. Its dry run creates nothing — **not even the property**, which would otherwise change the CRM schema while claiming to change nothing — and its real output is the area-code distribution, which is what tells you whether the phone data is good enough for the filter to be worth using at all.
> - **`backfillDealCampaign` skips rather than stamping blank.** A deal whose contact has no campaign is left alone, because an empty string in HubSpot means "clear this property" and would be indistinguishable from a real value later. Its search filters on `NOT_HAS_PROPERTY`, which is what makes the whole run idempotent: an already-stamped deal is not returned, so a re-run costs one empty search.
> - **`backfillEmailSuppression` skips a failing SendGrid list without losing the Firestore passes.** Losing the vendor's view of history must not cost us our own. The three passes overlap freely — a customer who replied "stop" is usually also on SendGrid's global unsubscribe list — because `suppress` merges on a lowercased-address doc id, first reason winning and later sources appending.
> - **`run_deal_attribution` pages to EXHAUSTION**, the opposite bound to the HTTP endpoint running the same scan. Deliberate: a scheduler wants a bounded slice because a request that runs for an hour gets killed; an operator wants the job finished. `onlyChatId` short-circuits after one page even if a cursor came back — a single-chat verification has nothing to page through.
> - **`reconcileStaleCalls` passes `null` for the age**, not a number, so the service keeps ownership of the default and the two cannot drift. It exists for the thing the cron cannot do: hand-picked bounds, so a shorter window after a known provider outage finds calls the cron's conservative threshold is still waiting on.
> - **One of my own tests was misleading and got fixed.** "Stops on a failed search" never reached the failure — a short first page ended the walk before the second request. Corrected to a full first page, and a separate test now pins the short-page exit.
> - **Files:**
>   - `outbound/commands/optoutBackfills.ts`, `outbound/commands/hubspotBackfills.ts`, `outbound/commands/index.ts`
>   - `outbound/services/emailCompliance.ts` (`CHAT_EVENT_DISPOSITION` promoted from a test-only export — the suppression backfill maps a stored class onto the same three outcomes)
>   - `outbound/__tests__/commands/{optoutBackfills,hubspotBackfills,index}.test.ts`
> - **Verification:** 2,239 tests across 55 suites (66 new), `tsc --noEmit` clean, `eslint` clean.
>
> ---
>
